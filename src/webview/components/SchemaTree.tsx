...
import type { SchemaTreeNode, SchemaNodeType } from '../../shared/types/schema'
...
export const SchemaTree: React.FC<SchemaTreeProps> = ({
  ...
  const handleNodeClick = useCallback((node: SchemaTreeNode, event: React.MouseEvent) => {
    event.stopPropagation()
    onNodeClick?.(node)
  }, [onNodeClick])

  const handleNodeDoubleClick = useCallback((node: SchemaTreeNode, event: React.MouseEvent) => {
    event.stopPropagation()
    onNodeDoubleClick?.(node)
  }, [onNodeDoubleClick])

  const handleContextMenu = useCallback((node: SchemaTreeNode, event: React.MouseEvent) => {
    ...
    <div className='schema-tree-node'>
      ...
      >
        <div className='schema-tree-item-content'>
          ...
          <div className='schema-tree-icon'>
            {nodeIcon}
          </div>
          
          <span className='schema-tree-label'>
            {node.label}
          </span>
...
const LoadingSpinner: React.FC<{ size?: 'small' | 'medium' | 'large' }> = ({ size = 'medium' }) => {
  ...
  return (
    <div className={`animate-spin rounded-full border-2 border-gray-300 border-t-blue-400 ${sizeClass}`} />
  )
}
...
const ColumnIcon: React.FC = () => (
  <svg className='w-4 h-4 text-gray-400' fill='currentColor' viewBox='0 0 20 20'>
    ...
    <path fillRule='evenodd' d='M17 16a1 1 0 01-1 1h-4a1 1 0 010-2h1.586l-2.293-2.293a1 1 0 011.414-1.414L15 13.586V12a1 1 0 012 0v4z' clipRule='evenodd' />
  </svg>
)
