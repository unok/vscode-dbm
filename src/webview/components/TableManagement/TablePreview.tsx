...
export const TablePreview: React.FC<TablePreviewProps> = ({
  ...
  const primaryKeyColumns = tableDefinition.columns.filter(col => col.isPrimaryKey)
  const foreignKeyConstraints = tableDefinition.constraints?.filter(c => c.type === "FOREIGN_KEY") || []
  const uniqueConstraints = tableDefinition.constraints?.filter(c => c.type === "UNIQUE") || []
  const checkConstraints = tableDefinition.constraints?.filter(c => c.type === "CHECK") || []
  const uniqueIndexes = tableDefinition.indexes?.filter(idx => idx.unique) || []
  const regularIndexes = tableDefinition.indexes?.filter(idx => !idx.unique) || []
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {activeTab === "summary" && (
          <div className="space-y-4">
            {/* Table Info */}
            <div>
              <h4 className="text-sm font-medium text-gray-900 mb-2">Table Information</h4>
              <div className="text-sm text-gray-600 space-y-1">
                <div>
                  <span className="font-medium">Name:</span> {tableDefinition.name || "Untitled"}
                </div>
                <div>
                  <span className="font-medium">Schema:</span> {tableDefinition.schema}
                </div>
                {tableDefinition.comment && (
                  <div>
                    <span className="font-medium">Comment:</span> {tableDefinition.comment}
                  </div>
                )}
              </div>
            </div>
            ...
            {primaryKeyColumns.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-gray-900 mb-2">Primary Key</h4>
                ...
                <div className="space-y-2">
                  {uniqueIndexes.map((idx, index) => (
                    <div key={`unique-${index}`} className="text-xs bg-gray-50 p-2 rounded">
                      ...
                      {idx.where && (
                        <div className="text-gray-500 font-mono">WHERE {idx.where}</div>
                      )}
                    </div>
                  ))}
          ...
          <div className="space-y-4">
            ...
            {isGenerating ? (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2 text-sm text-gray-600">Generating SQL...</span>
              </div>
            ) : generatedSQL ? (
