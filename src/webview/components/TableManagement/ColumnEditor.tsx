          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {columns.map((column, index) => {
                const { type, length, precision, scale } = parseDataType(
                  column.dataType,
                );
      ...
      {columns.length > 0 && (
        <div className="mt-4 p-4 bg-blue-50 rounded-md">
          ...
          <ul className="text-sm text-blue-800 space-y-1">
            ...
            <li>
              • Auto increment is only available for integer primary key columns
            </li>
            <li>
              • Use VARCHAR for variable-length text, CHAR for fixed-length
            </li>
            <li>
              • DECIMAL/NUMERIC requires precision (total digits) and scale
              (decimal places)
            </li>
          </ul>
        </div>
