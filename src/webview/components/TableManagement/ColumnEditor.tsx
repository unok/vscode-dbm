...
export const ColumnEditor: React.FC<ColumnEditorProps> = ({
  ...
  const availableDataTypes = Object.values(DATA_TYPE_MAPPINGS[databaseType as keyof typeof DATA_TYPE_MAPPINGS] || {})
                  ...
                  <tr key={index} className={isEditing ? "bg-blue-50" : "hover:bg-gray-50"}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <select
                        value={type}
                        onChange={(e) => {
                          const newDataType = buildDataType(e.target.value, length, precision, scale)
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {type.includes("VARCHAR") || type.includes("CHAR") ? (
                        <input
                          ...
                          value={length}
                          onChange={(e) => {
                            const newDataType = buildDataType(type, e.target.value, precision, scale)
                            updateColumn(index, "dataType", newDataType)
                          }}
                        ...
                        <div className="flex space-x-1">
                          ...
                          <input
                            ...
                            value={scale}
                            onChange={(e) => {
                              const newDataType = buildDataType(type, length, precision, e.target.value)
                              updateColumn(index, "dataType", newDataType)
                            }}
                            className="w-16 px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                            placeholder="2"
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <input
                        ...
                        disabled={!column.isPrimaryKey || !["INT", "INTEGER", "BIGINT", "SERIAL", "BIGSERIAL"].some(t => type.includes(t))}
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      />
                    </td>
