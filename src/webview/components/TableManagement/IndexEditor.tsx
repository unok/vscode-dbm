...
export const IndexEditor: React.FC<IndexEditorProps> = ({
  ...
  const indexTypes: { value: IndexDefinition["type"]; label: string; description: string }[] = [
    { value: "BTREE", label: "B-Tree", description: "Default balanced tree index for most queries" },
  ...
  const updateIndex = useCallback((index: number, field: keyof IndexDefinition, value: any) => {
  ...
  const updateIndexColumns = useCallback((index: number, selectedColumns: string[]) => {
    updateIndex(index, "columns", selectedColumns)
  }, [updateIndex])

  const updateIncludeColumns = useCallback((index: number, selectedColumns: string[]) => {
    updateIndex(index, "include", selectedColumns)
  }, [updateIndex])

  const moveColumnUp = useCallback((indexIdx: number, columnIdx: number) => {
    if (columnIdx === 0) return
    
    const currentIndex = indexes[indexIdx]
    const newColumns = [...currentIndex.columns]
    ;[newColumns[columnIdx - 1], newColumns[columnIdx]] = [newColumns[columnIdx], newColumns[columnIdx - 1]]
    updateIndexColumns(indexIdx, newColumns)
  }, [indexes, updateIndexColumns])

  const moveColumnDown = useCallback((indexIdx: number, columnIdx: number) => {
    const currentIndex = indexes[indexIdx]
    if (columnIdx === currentIndex.columns.length - 1) return
    
    const newColumns = [...currentIndex.columns]
    ;[newColumns[columnIdx], newColumns[columnIdx + 1]] = [newColumns[columnIdx + 1], newColumns[columnIdx]]
    updateIndexColumns(indexIdx, newColumns)
  }, [indexes, updateIndexColumns])

  const removeColumnFromIndex = useCallback((indexIdx: number, columnName: string) => {
    const currentIndex = indexes[indexIdx]
    ...
    <div className="space-y-4">
      ...
      {indexes.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          <p>No indexes defined. Add columns first, then create indexes to improve query performance.</p>
        </div>
      ) : (
        <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => updateIndex(indexIdx, "type", e.target.value as IndexDefinition["type"])}
                        ...
                        {index.columns.length > 0 && (
                          <div className="mb-3 space-y-2">
                            {index.columns.map((columnName, columnIdx) => (
                              <div key={columnName} className="flex items-center space-x-2 p-2 bg-gray-50 rounded">
                      ...
                      {features.supportsPartialIndexes && (
                        <div>
                          ...
                          <input
                            type="text"
                            value={index.where || ""}
                            onChange={(e) => updateIndex(indexIdx, "where", e.target.value || undefined)}
                            className="w-full px-3 py-2 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={index.comment || ""}
                          onChange={(e) => updateIndex(indexIdx, "comment", e.target.value || undefined)}
                          className="w-full px-3 py-2 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
