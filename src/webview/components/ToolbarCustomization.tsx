...
export const ToolbarCustomization: React.FC<ToolbarCustomizationProps> = ({
  ...
  const [layout, setLayout] = useState<ToolbarLayout>(
    toolbarService.getLayout(),
  );
  const [hiddenItems, setHiddenItems] = useState<ToolbarItem[]>(
    toolbarService.getHiddenItems(),
  );
  const [draggedItem, setDraggedItem] = useState<DragItem | null>(null);
  const [activeTab, setActiveTab] = useState<"layout" | "appearance" | "items">(
    "layout",
  );
  ...
  const handleResetToDefault = useCallback(() => {
    if (
      confirm(
        "Are you sure you want to reset all toolbar customizations to default?",
      )
    ) {
  ...
  const handleDragStart = useCallback(
    (item: ToolbarItem, sourceSection: string) => {
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Drag and drop items between sections to customize your toolbar
                  layout.
                </p>
          ...
          {activeTab === "items" && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Toggle visibility of toolbar items. Hidden items can be
                  restored here.
                </p>
                      ...
                      {hiddenItems.map((item) => (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              handleItemVisibilityToggle(item.id, true)
                            }
