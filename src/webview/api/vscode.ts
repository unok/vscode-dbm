...
class VsCodeWebViewApi {
  ...
  private initialize() {
    ...
    try {
      ...
      if (
        ...
        (window as unknown as { vscode?: VsCodeApi }).vscode
      ) {
        this.vscode =
          (window as unknown as { vscode?: VsCodeApi }).vscode || null;
      ...
      if (
        this.isInitialized &&
        !window.addEventListener.toString().includes("handleMessage")
      ) {
  ...
  private setupDevMode() {
    ...
    console.error(
      "VSCode API is required. Please run this extension within VSCode.",
    );
  ...
  public async openConnection(
    ...
    return new Promise((resolve) => {
      this.onMessage("connectionResult", (data) =>
        resolve(data as ConnectionResult),
      );
