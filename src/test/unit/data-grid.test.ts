...
import type { TableData, ColumnDefinition, CellValue, EditableCell } from '../../shared/types/datagrid'
...
describe('DataGridService', () => {
  ...
  describe('Table Data Loading', () => {
    it('should load table data correctly', async () => {
      // Act
      const result = await dataGridService.loadTableData('users', { offset: 0, limit: 50 })
    ...
    it('should handle pagination parameters', async () => {
      // Act
      const result = await dataGridService.loadTableData('users', { offset: 10, limit: 25 })
    ...
    it('should handle empty tables', async () => {
      ...
      // Act
      const result = await dataGridService.loadTableData('empty_table', { offset: 0, limit: 50 })
    ...
    it('should throw error for non-existent table', async () => {
      // Act & Assert
      await expect(dataGridService.loadTableData('non_existent', { offset: 0, limit: 50 }))
  ...
  describe('Column Definition Processing', () => {
    it('should process column definitions correctly', () => {
      // Act
      const columns = dataGridService.processColumnDefinitions(mockTableData.columns)
  ...
  describe('Cell Editing', () => {
    it('should validate cell value types', () => {
      ...
      expect(dataGridService.validateCellValue('abc', integerColumn)).toBe(false)
    ...
    it('should handle nullable columns correctly', () => {
      ...
      expect(dataGridService.validateCellValue(null, nullableColumn)).toBe(true)
    })
    ...
    it('should prevent editing of non-editable columns', () => {
      ...
      // Act & Assert
      expect(() => dataGridService.updateCellValue(0, 'id', '999'))
        .toThrow('Column "id" is not editable')
    })
  })
...
describe('UUIDGenerator', () => {
  ...
  describe('UUID v7 Generation', () => {
    it('should generate valid UUID v7 with timestamp ordering', () => {
      ...
      // Assert
      expect(uuid1).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i)
      expect(uuid2).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i)
  ...
  describe('Default Value Generation', () => {
    it('should generate appropriate default values for column types', () => {
      ...
      expect(uuidDefault).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i)
    })
    ...
    it('should generate current timestamp for datetime columns', () => {
      ...
      expect(Math.abs(Date.now() - new Date(defaultValue as string).getTime())).toBeLessThan(1000)
    })
  })
})
