...
describe("TransactionService", () => {
  ...
  describe("Transaction Lifecycle", () => {
    test("should begin a new transaction", async () => {
      ...
      const result = await transactionService.beginTransaction(mockContext, options)
    ...
    test("should commit a transaction", async () => {
      const beginResult = await transactionService.beginTransaction(mockContext)
      ...
      const transaction = transactionService.getTransaction(beginResult.transactionId)
    ...
    test("should not commit non-existent transaction", async () => {
      const result = await transactionService.commitTransaction("non_existent", mockContext)
    ...
    test("should not rollback non-existent transaction", async () => {
      const result = await transactionService.rollbackTransaction("non_existent", mockContext)
  ...
  describe("Statement Execution in Transaction", () => {
    ...
    test("should auto-commit when autoCommit is enabled", async () => {
      ...
      const beginResult = await transactionService.beginTransaction(mockContext, options)
      const sql = "INSERT INTO users (name, email) VALUES ('John', 'john@example.com')"
    ...
    test("should not execute in inactive transaction", async () => {
      const beginResult = await transactionService.beginTransaction(mockContext)
      await transactionService.commitTransaction(beginResult.transactionId, mockContext)
  ...
  describe("Savepoints", () => {
    ...
    test("should not create duplicate savepoint", async () => {
      ...
      await transactionService.createSavepoint(beginResult.transactionId, "sp1", mockContext)
    ...
    test("should rollback to savepoint", async () => {
      ...
      let transaction = transactionService.getTransaction(beginResult.transactionId)
      ...
      transaction = transactionService.getTransaction(beginResult.transactionId)
    ...
    test("should release savepoint", async () => {
      ...
      await transactionService.createSavepoint(beginResult.transactionId, "sp1", mockContext)
      await transactionService.createSavepoint(beginResult.transactionId, "sp2", mockContext)
    ...
    test("should not rollback to non-existent savepoint", async () => {
      ...
      expect(rollbackResult.error).toContain("Savepoint 'non_existent' not found")
    })
  })
  ...
  describe("Transaction Management", () => {
    test("should get active transactions", async () => {
      ...
      await transactionService.commitTransaction(tx2.transactionId, mockContext)
    ...
    test("should clean up old transactions", async () => {
      ...
      await transactionService.commitTransaction(tx1.transactionId, mockContext)
      await transactionService.rollbackTransaction(tx2.transactionId, mockContext)
      ...
      if (transaction1) transaction1.endTime = new Date(Date.now() - 25 * 60 * 60 * 1000) // 25 hours ago
      if (transaction2) transaction2.endTime = new Date(Date.now() - 25 * 60 * 60 * 1000) // 25 hours ago
  ...
  describe("Error Scenarios", () => {
    ...
    test("should not operate on non-active transactions", async () => {
      ...
      const rollbackCommittedResult = await transactionService.rollbackTransaction(
        beginResult.transactionId,
        mockContext
      )
  ...
  describe("Transaction State Tracking", () => {
    ...
    test("should track transaction timing", async () => {
      ...
      expect(transaction?.endTime).toBeTruthy()
      expect(transaction?.endTime?.getTime()).toBeGreaterThan(transaction?.startTime.getTime())
    })
  })
})
