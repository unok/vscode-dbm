...
describe("SettingsUI Simplified", () => {
  ...
  it("should render settings dialog", () => {
    render(<SettingsUI settingsService={settingsService} onClose={mockOnClose} />)
  ...
  it("should switch to database settings", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Connection timeout (seconds)")).toBeInTheDocument()
  ...
  it("should save settings and close when save is clicked", async () => {
    ...
    // Toggle a setting
    const autoSaveCheckbox = screen.getByRole("checkbox", { name: /auto-save queries/i })
