...
describe("DataExportService", () => {
  ...
  describe("CSV Export", () => {
    test("should export to CSV with headers", async () => {
      ...
      expect(lines[1]).toBe(
        "1,john@example.com,John Doe,true,2023-01-01T10:00:00Z",
      );
      expect(lines[2]).toBe(
        "2,jane@example.com,Jane Smith,false,2023-01-02T11:00:00Z",
      );
  ...
  describe("File Operations", () => {
    test("should calculate export size correctly", () => {
      const estimatedSize = exportService.estimateExportSize(
        mockTableData,
        "csv",
      );
