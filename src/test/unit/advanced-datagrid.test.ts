...
describe("AdvancedDataGridService", () => {
  ...
  describe("Advanced Inline Editing", () => {
    ...
    test("should validate cell values during editing", async () => {
      ...
      const validationResult = await advancedDataGrid.validateCellValue(
        0,
        "email",
        "invalid-email",
      );
    ...
    test("should commit valid cell changes", async () => {
      ...
      const result = await advancedDataGrid.commitCellEdit(
        0,
        "email",
      ...
      expect(result.success).toBe(true);
      expect(advancedDataGrid.getCellValue(0, "email")).toBe(
        "newemail@example.com",
      );
    });
  ...
  describe("Visual Change Indicators", () => {
    ...
    test("should support custom visual indicators", async () => {
      advancedDataGrid.setCustomIndicator(
        0,
        "email",
        "warning",
  ...
  describe("Copy and Paste Functionality", () => {
    test("should copy single cell", () => {
      const copyData = advancedDataGrid.copyCells([
    ...
    test("should paste single cell", async () => {
      ...
      const result = await advancedDataGrid.pasteCells(copyData, {
        rowIndex: 1,
        columnId: "email",
      });
      ...
      expect(advancedDataGrid.getCellValue(1, "email")).toBe(
        "newemail@example.com",
      );
    });
    ...
    test("should paste range with auto-expansion", async () => {
      ...
      expect(advancedDataGrid.getCellValue(2, "email")).toBe(
        "new2@example.com",
      );
    ...
    test("should support clipboard integration", async () => {
      ...
      // Copy to clipboard
      await advancedDataGrid.copyToClipboard([
        { rowIndex: 0, columnId: "email" },
      ]);
      expect(mockClipboard.writeText).toHaveBeenCalledWith("john@example.com");

      // Paste from clipboard
      const result = await advancedDataGrid.pasteFromClipboard({
        rowIndex: 1,
        columnId: "email",
      });
      expect(result.success).toBe(true);
  ...
  describe("Cursor AI Integration", () => {
    test("should generate smart default values", async () => {
      ...
      vi.spyOn(aiIntegration, "generateDefaults").mockResolvedValue(
        mockAiResponse,
      );
