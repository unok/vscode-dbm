...
import { BookmarkManager, type QueryBookmark } from "../../shared/services/BookmarkManager"
import {
...
import {
  ...
  type QueryPlan,
  QueryPlanAnalyzer,
} from "../../shared/services/QueryPlanAnalyzer"
import { type FormatOptions, SQLFormatter } from "../../shared/services/SQLFormatter"
import type { DatabaseSchema, QueryExecutionContext } from "../../shared/types/sql"
...
describe("AdvancedSQLAutoCompleter", () => {
      ...
      tables: [
        {
          ...
          columns: [
            ...
            { name: "first_name", type: "varchar(100)", nullable: true, isPrimaryKey: false },
            { name: "last_name", type: "varchar(100)", nullable: true, isPrimaryKey: false },
        ...
        },
        {
          ...
          columns: [
            ...
            { name: "amount", type: "decimal(10,2)", nullable: false, isPrimaryKey: false },
            { name: "status", type: "varchar(50)", nullable: false, isPrimaryKey: false },
            { name: "order_date", type: "timestamp", nullable: false, isPrimaryKey: false },
          ],
        },
      ],
      views: [],
      procedures: [],
      functions: ["COUNT", "SUM", "AVG", "MAX", "MIN", "CONCAT", "UPPER", "LOWER"],
    }
  ...
  describe("Table and Column Completion", () => {
    test("should complete table names after FROM keyword", async () => {
      ...
      const context: CompletionContext = { sql, position, cursorPosition: position }
    ...
    test("should complete column names after SELECT keyword", async () => {
      ...
      expect(result.suggestions.some((s) => s.text === "users.email")).toBe(true)
    })
    ...
    test("should complete JOIN syntax", async () => {
      ...
      expect(result.suggestions.some((s) => s.text.includes("INNER JOIN"))).toBe(true)
      expect(result.suggestions.some((s) => s.text.includes("LEFT JOIN"))).toBe(true)
      expect(result.suggestions.some((s) => s.text.includes("RIGHT JOIN"))).toBe(true)
    })
  })
  ...
  describe("Function Completion", () => {
    test("should complete SQL functions", async () => {
      ...
      expect(result.suggestions.some((s) => s.text === "COUNT" && s.type === "function")).toBe(true)
      expect(result.suggestions.some((s) => s.text === "CONCAT" && s.type === "function")).toBe(
        true
      )
    })
    ...
    test("should provide function templates with parameters", async () => {
      ...
      expect(result.suggestions.some((s) => s.text.includes("DISTINCT"))).toBe(true)
    })
  })
  ...
  describe("Subquery Completion", () => {
    test("should complete subquery context", async () => {
      ...
      expect(result.suggestions.some((s) => s.text.startsWith("SELECT"))).toBe(true)
...
describe("QueryHistoryManager", () => {
  ...
  describe("History Storage", () => {
    ...
    test("should store multiple queries with timestamps", async () => {
      await historyManager.addEntry("SELECT * FROM users", mockContext, { rowCount: 10 })
      await historyManager.addEntry("SELECT * FROM orders", mockContext, { rowCount: 5 })
    ...
    test("should limit history size", async () => {
      ...
      for (let i = 0; i < 1100; i++) {
        await historyManager.addEntry(`SELECT ${i}`, mockContext, { rowCount: 1 })
      }
  ...
  describe("History Search", () => {
    beforeEach(async () => {
      ...
      await historyManager.addEntry("UPDATE users SET last_name = 'Smith'", mockContext, {
        rowCount: 5,
      })
      await historyManager.addEntry("SELECT COUNT(*) FROM orders", mockContext, { rowCount: 1 })
    })
