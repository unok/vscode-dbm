...
import { useVSCodeAPI } from "@/webview/api/vscode"
...
describe("ConnectionConfigDialog", () => {
  ...
  beforeEach(() => {
    mockOnSave = vi.fn()
    mockOnCancel = vi.fn()
    mockOnTest = vi.fn().mockResolvedValue({ success: true, message: "Connection successful" })
  ...
  describe("接続の編集", () => {
    ...
    it("既存の設定が正しく表示される", () => {
      ...
      expect((screen.getByLabelText(/Database Name/) as HTMLInputElement).value).toBe("testdb")
    })
  })
  ...
  describe("バリデーション", () => {
    it("必須フィールドが空の場合エラーが表示される", () => {
      ...
      expect(mockVSCodeAPI.showError).toHaveBeenCalledWith("Connection name is required")
