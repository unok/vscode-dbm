  ...
  describe("データベース設定の暗号化・復号化テスト", () => {
    it("データベース設定全体の暗号化・復号化が正常に動作する", () => {
      const encryptedConfig =
        encryptionService.encryptDatabaseConfig(mockConfig);
      ...
      const decryptedConfig =
        encryptionService.decryptDatabaseConfig(encryptedConfig);
    ...
    it("複数のデータベース設定を暗号化・復号化できる", () => {
      ...
      const encryptedConfigs =
        encryptionService.encryptMultipleDatabaseConfigs(configs);
      const decryptedConfigs =
        encryptionService.decryptMultipleDatabaseConfigs(encryptedConfigs);
