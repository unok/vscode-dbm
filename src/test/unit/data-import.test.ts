...
jane@example.com,Jane Smith,2023-01-02T11:00:00Z`
      ...
      const mockFile = new File([csvContent], "users.csv", { type: "text/csv" })
...
"jane@example.com","Jane, Smith","2023-01-02T11:00:00Z"`
    ...
    test("should handle empty CSV file", async () => {
      ...
      await expect(importService.importData(mockFile, options, mockTableData)).rejects.toThrow(
        "Empty CSV file"
      )
    })
  })
  ...
  describe("JSON Import", () => {
    test("should import JSON array", async () => {
      const jsonContent = JSON.stringify([
        { email: "john@example.com", name: "John Doe", created_at: "2023-01-01T10:00:00Z" },
        { email: "jane@example.com", name: "Jane Smith", created_at: "2023-01-02T11:00:00Z" },
      ])

      const mockFile = new File([jsonContent], "users.json", { type: "application/json" })
      const options: ImportOptions = {
        format: "json",
        hasHeaders: false,
        onConflict: "ignore",
    ...
    test("should import single JSON object", async () => {
      ...
      const mockFile = new File([jsonContent], "user.json", { type: "application/json" })
      const options: ImportOptions = {
        format: "json",
        hasHeaders: false,
        onConflict: "ignore",
...
INSERT INTO users (email, name, created_at) VALUES ('jane@example.com', 'Jane Smith', '2023-01-02T11:00:00Z');`
      ...
      const mockFile = new File([sqlContent], "users.sql", { type: "text/sql" })
      const options: ImportOptions = {
        format: "sql",
        hasHeaders: false,
        onConflict: "ignore",
...
SELECT * FROM users;`
      ...
      const mockFile = new File([sqlContent], "no-inserts.sql", { type: "text/sql" })
      const options: ImportOptions = {
        format: "sql",
        hasHeaders: false,
        onConflict: "ignore",
...
jane@example.com,Jane Smith,invalid-date`
      ...
      const result = await importService.importData(mockFile, options, tableWithValidation)
...
jane@example.com,Jane Smith,2023-01-02T11:00:00Z`
      ...
      const preview = await importService.previewImport(mockFile, options, mockTableData)
...
john@example.com,John Doe,1990-01-01`
      ...
      expect(preview.conflicts.some((c) => c.type === "column_mismatch")).toBe(true)
    })
  })
  ...
  describe("Import Progress", () => {
    test("should report import progress", async () => {
      ...
      const mockFile = new File([csvWithHeaders], "large.csv", { type: "text/csv" })
      const options: ImportOptions = {
        format: "csv",
        hasHeaders: true,
        onConflict: "ignore",
      ...
      const result = await importService.importData(mockFile, options, mockTableData, onProgress)
...
2,200`
      ...
      const mockFile = new File([csvContent], "stats.csv", { type: "text/csv" })
      const options: ImportOptions = {
        format: "csv",
        hasHeaders: true,
        onConflict: "ignore",
      ...
      const result = await importService.importData(mockFile, options, tableData)
