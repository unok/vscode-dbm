...
import type { DatabaseColumn, DatabaseTable } from "@/shared/types/schema"
...
describe("AdvancedSQLAutoCompleter", () => {
  ...
  describe("キーワード補完", () => {
    ...
    test("FROM句の後にテーブル名を提案する", async () => {
      const suggestions = await autoCompleter.getSuggestions("SELECT * FROM ", 14)
  ...
  describe("テーブル補完", () => {
    test("テーブル名の部分一致で補完候補を絞り込む", async () => {
      const suggestions = await autoCompleter.getSuggestions("SELECT * FROM us", 16)
    ...
    test("エイリアス付きテーブルの補完", async () => {
      const suggestions = await autoCompleter.getSuggestions("SELECT * FROM users u, ", 23)
  ...
  describe("カラム補完", () => {
    test("テーブル名の後にカラムを提案する", async () => {
      const suggestions = await autoCompleter.getSuggestions("SELECT users.", 13)
    ...
    test("JOIN句でのカラム補完", async () => {
      ...
      const suggestions = await autoCompleter.getSuggestions(context, context.length)
    ...
    test("集計関数の補完", async () => {
      ...
      expect(suggestions).toContainEqual(
        expect.objectContaining({
          ...
          insertTextRules: mockMonaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
        })
      )
    })
  ...
  describe("複雑なクエリでの補完", () => {
    ...
    test("複数JOIN文での補完", async () => {
      const context = `
        ...
        WHERE u.
      `
      const suggestions = await autoCompleter.getSuggestions(context, context.lastIndexOf("u.") + 2)
  ...
  describe("パフォーマンス", () => {
    test("大量のテーブルでも高速に補完候補を返す", async () => {
      ...
      const suggestions = await autoCompleter.getSuggestions("SELECT * FROM table_5", 21)
  ...
  describe("エラーハンドリング", () => {
    test("メタデータ取得エラー時は基本的な補完のみ提供", async () => {
      vi.mocked(mockMetadataService.getTables).mockRejectedValue(new Error("Connection error"))
      ...
      // テーブル補完は空
      const tableSuggestions = await autoCompleter.getSuggestions("SELECT * FROM ", 14)
