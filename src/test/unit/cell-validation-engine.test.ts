...
describe("CellValidationEngine", () => {
  ...
  describe("Boolean Validation", () => {
    ...
    test("should validate string boolean values", async () => {
      const trueResult = await validationEngine.validateCell(
        0,
        "active",
        "true",
      );
      const falseResult = await validationEngine.validateCell(
        0,
        "active",
        "false",
      );
  ...
  describe("JSON Validation", () => {
    test("should validate valid JSON objects", async () => {
      const result = await validationEngine.validateCell(0, "data", {
        key: "value",
        number: 123,
      });
  ...
  describe("Business Logic Validation", () => {
    test("should validate unique email addresses", async () => {
      ...
      const validResult = await validationEngine.validateCell(
        0,
        "email",
        "new@example.com",
      );
      ...
      const duplicateResult = await validationEngine.validateCell(
        0,
        "email",
        "john@example.com",
      );
    ...
    test("should validate date ranges", async () => {
      ...
      const tooEarlyResult = await validationEngine.validateCell(
        0,
        "created_at",
        "2019-01-01",
      );
      ...
      const tooLateResult = await validationEngine.validateCell(
        0,
        "created_at",
        "2031-01-01",
      );
  ...
  describe("Schema Updates", () => {
    test("should update schema and validate accordingly", async () => {
      ...
      const oldResult = await validationEngine.validateCell(
        0,
        "email",
        "test@example.com",
      );
