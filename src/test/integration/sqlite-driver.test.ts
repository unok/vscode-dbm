  ...
  describe("SQLite固有機能テスト", () => {
    ...
    it("JSON関数の操作が正常に動作する", async () => {
      ...
      const jsonData = {
        name: "テストユーザー",
        age: 30,
        tags: ["developer", "sqlite"],
      };

      await driver.query("INSERT INTO test_table (data) VALUES (?)", [
    ...
    it("BLOB型データの操作が正常に動作する", async () => {
      ...
      await driver.query("INSERT INTO test_table (binary_data) VALUES (?)", [
        binaryData,
      ]);

      const result = await driver.query(
        "SELECT binary_data FROM test_table WHERE id = 1",
      );
      expect(Buffer.from(result.rows[0].binary_data).toString("utf8")).toBe(
        "テストバイナリデータ",
      );
    });
    ...
    it("トランザクションが正常に動作する", async () => {
      ...
      try {
        await driver.query(
          "UPDATE test_table SET balance = balance - 50 WHERE id = 1",
        );
        ...
        const result = await driver.query(
          "SELECT SUM(balance) as total FROM test_table",
        );
