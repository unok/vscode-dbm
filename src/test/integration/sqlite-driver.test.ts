...
describe('SQLiteDriver統合テスト', () => {
  ...
  describe('SQLite固有機能テスト', () => {
    ...
    it('JSON関数の操作が正常に動作する', async () => {
      ...
      // JSON関数を使用してデータ取得
      const result = await driver.query('SELECT json_extract(data, "$.name") as name FROM test_table WHERE id = 1');
    ...
    it('BLOB型データの操作が正常に動作する', async () => {
      ...
      await driver.query(
        'INSERT INTO test_table (binary_data) VALUES (?)',
        [binaryData]
      );
    ...
    it('FTS（全文検索）の操作が正常に動作する', async () => {
      ...
      await driver.query(
        ...
        ['SQLiteドキュメント', 'SQLiteは軽量で高速なデータベースエンジンです']
      );
      
      const result = await driver.query('SELECT * FROM documents WHERE documents MATCH ?', ['SQLite']);
      expect(result.rows).toHaveLength(1);
  ...
  describe('エラーハンドリングテスト', () => {
    ...
    it('制約違反でエラーが適切に処理される', async () => {
      ...
      // 重複挿入
      const result = await driver.query('INSERT INTO test_table (email) VALUES (?)', ['test@example.com']);
    ...
    it('型制約違反でエラーが適切に処理される', async () => {
      ...
      const result = await driver.query('INSERT INTO test_table (value) VALUES (?)', ['not_a_number']);
