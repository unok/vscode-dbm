...
describe('PostgreSQLDriver統合テスト', () => {
  ...
  describe('基本クエリテスト', () => {
    ...
    it('UUID生成クエリが正常に実行される', async () => {
      ...
      expect(result.rows).toHaveLength(1);
      expect(result.rows[0].uuid_value).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
    });
  ...
  describe('PostgreSQL固有機能テスト', () => {
    ...
    it('ARRAY型データの操作が正常に動作する', async () => {
      ...
      await driver.query(
        'INSERT INTO test_table (tags) VALUES ($1)',
        [tags]
      );
    ...
    it('UUIDカラムの操作が正常に動作する', async () => {
      ...
      expect(result.rows).toHaveLength(1);
      expect(result.rows[0].id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
  ...
  describe('エラーハンドリングテスト', () => {
    ...
    it('制約違反でエラーが適切に処理される', async () => {
      ...
      // 重複挿入
      const result = await driver.query('INSERT INTO test_table (email) VALUES ($1)', ['test@example.com']);
