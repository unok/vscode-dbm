...
describe("PostgreSQLDriver統合テスト", () => {
  ...
  describe("基本クエリテスト", () => {
    ...
    it("データ削除クエリが正常に実行される", async () => {
      ...
      await driver.query("INSERT INTO test_table (name) VALUES ($1)", [
        "テストユーザー",
      ]);
  ...
  describe("PostgreSQL固有機能テスト", () => {
    ...
    it("JSON型データの操作が正常に動作する", async () => {
      ...
      await driver.query("INSERT INTO test_table (data) VALUES ($1)", [
    ...
    it("ARRAY型データの操作が正常に動作する", async () => {
      ...
      const result = await driver.query(
        "SELECT tags FROM test_table WHERE id = 1",
      );
    ...
    it("ENUMタイプの操作が正常に動作する", async () => {
      ...
      await driver.query("INSERT INTO test_table (status) VALUES ($1)", [
        "active",
      ]);

      const result = await driver.query(
        "SELECT status FROM test_table WHERE id = 1",
      );
  ...
  describe("エラーハンドリングテスト", () => {
    ...
    it("制約違反でエラーが適切に処理される", async () => {
      ...
      await driver.query("INSERT INTO test_table (email) VALUES ($1)", [
        "test@example.com",
      ]);

      // 重複挿入
      const result = await driver.query(
        "INSERT INTO test_table (email) VALUES ($1)",
        ["test@example.com"],
      );
  ...
  describe("パフォーマンステスト", () => {
    ...
    it("大量データ挿入が適切な時間で完了する", async () => {
      ...
      await driver.query(
        `INSERT INTO test_table (name, value) VALUES ${values}`,
      );
