...
export class DataImportService {
   ...
   * Parse CSV data
   */
  private parseCSV(text: string, options: ImportOptions): Record<string, CellValue>[] {
   ...
   * Parse JSON data
   */
  private parseJSON(text: string, _options: ImportOptions): Record<string, CellValue>[] {
    try {
   ...
   */
  private parseSQL(text: string, _options: ImportOptions): Record<string, CellValue>[] {
  ...
  private generatePreview(
    ...
    if (targetTable) {
      ...
      const missingColumns = columns.filter((col) => !targetColumns.includes(col))
      if (missingColumns.length > 0) {
        conflicts.push({
          type: "column_mismatch",
          message: `Columns not found in target table: ${missingColumns.join(", ")}`,
          suggestions: ["Add missing columns to target table", "Map columns during import"],
        })
      }

      // Check for extra columns in target
      const extraColumns = targetColumns.filter((col) => !columns.includes(col))
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      for (const column of targetTable.columns) {
        ...
        if (validation.warnings.length > 0) {
          warnings.push(`Row ${i + 1}, Column ${column.name}: ${validation.warnings.join(", ")}`)
        }
   ...
   */
  private transformValue(value: CellValue, column: ColumnDefinition): CellValue {
    if (value === null || value === undefined || value === "") {
    ...
    try {
      if (type.includes("int")) {
        return typeof value === "number" ? Math.floor(value) : Number.parseInt(String(value), 10)
      }
