...
export class AdvancedSQLAutoCompleter {
  ...
  constructor(
    ...
    this.functions = (schema.functions || []).map((f) => (typeof f === "string" ? f : f.name))
  }

  async getCompletions(context: CompletionContext): Promise<AutoCompleteResult> {
  ...
  private isExpectingColumn(tokens: string[]): boolean {
    ...
    // After SELECT
    if (lastToken === "select" || (lastToken === "distinct" && secondLastToken === "select")) {
      return true
    }
  ...
  private getKeywordSuggestions(context: CompletionContext): CompletionSuggestion[] {
    const sqlContext = this.analyzeSQLContext(context)
  ...
  private getFunctionSuggestions(_context: CompletionContext): CompletionSuggestion[] {
  ...
  private filterSuggestions(
    ...
    if (this.config.fuzzyMatching) {
      return suggestions.filter((suggestion) =>
        this.fuzzyMatch(suggestion.text.toLowerCase(), currentWord.toLowerCase())
      )
    }
    return suggestions.filter((suggestion) =>
  ...
  private isCompleteClause(tokens: string[]): boolean {
    ...
    return (
      tokensAfterKeyword.length > 0 && !tokensAfterKeyword.some((token) => this.isKeyword(token))
    )
  }
  ...
  private findTableByAlias(alias: string): DatabaseSchema["tables"][0] | undefined {
    // In a real implementation, this would track table aliases from the query
    // For now, return the first table that could match
    return this.schema.tables.find((table) => table.name.toLowerCase() === alias.toLowerCase())
  }
  ...
  private getColumnDocumentation(tableName: string, columnName: string): string {
