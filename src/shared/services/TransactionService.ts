...
export class TransactionService {
  ...
  async executeInTransaction(
    ...
    try {
      ...
      if (transaction.autoCommit && this.isModifyingStatement(sql)) {
        const commitResult = await this.commitTransaction(transactionId, context)
        return {
  ...
  async rollbackTransaction(
    ...
    try {
      ...
      if (savepointName) {
        const savepoint = transaction.savepoints.find((sp) => sp.name === savepointName)
        if (!savepoint) {
          return {
            success: false,
            transactionId,
        ...
        transaction.statements = transaction.statements.slice(0, savepoint.statements)
  ...
  async createSavepoint(
    ...
    try {
      const result = await this.executeTransactionControl(context, `SAVEPOINT ${savepointName}`)
  ...
  getActiveTransactions(): Transaction[] {
    return Array.from(this.transactions.values()).filter((tx) => tx.status === "active")
  }

  /**
   * Clean up completed transactions
   */
  cleanupTransactions(olderThan: Date = new Date(Date.now() - 24 * 60 * 60 * 1000)): number {
    let cleaned = 0
  ...
  getTransactionStats(): {
    ...
    const rolledBack = transactions.filter((tx) => tx.status === "rolled_back").length
    ...
    const avgExecutionTime =
      completedTransactions.length > 0
        ? completedTransactions.reduce((sum, tx) => {
            const duration = (tx.endTime?.getTime() || 0) - tx.startTime.getTime()
            return sum + duration
