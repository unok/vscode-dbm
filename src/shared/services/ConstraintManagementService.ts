...
export class ConstraintManagementService {
  ...
  generateDropConstraintSQL(
    ...
    switch (connection.type) {
      ...
      case "sqlite":
        throw new Error(
          "SQLite does not support dropping constraints. Table recreation required.",
        );
      default:
        return `ALTER TABLE ${this.escapeIdentifier(tableName)} DROP CONSTRAINT ${this.escapeIdentifier(constraintName)}`;
    }
  ...
  private validateForeignKeyConstraint(
    ...
    if (!constraint.columns || constraint.columns.length === 0) {
      errors.push({
        ...
        field: "columns",
        message:
          "Foreign key constraint must specify at least one local column",
        severity: "error",
      });
    }
    ...
    if (
      ...
      constraint.referencedTable.trim().length === 0
    ) {
      errors.push({
        type: "validation",
    ...
    if (
      ...
      errors.push({
        ...
        message:
          "Foreign key constraint must specify at least one referenced column",
        severity: "error",
      });
    }
    ...
    if (constraint.columns && constraint.referencedColumns) {
      if (constraint.columns.length !== constraint.referencedColumns.length) {
        errors.push({
          ...
          field: "columns",
          message:
            "Number of local columns must match number of referenced columns",
          severity: "error",
        });
      }
    }
  ...
  private validateCheckConstraint(
    ...
    if (
      ...
      constraint.checkExpression.trim().length === 0
    ) {
      errors.push({
        type: "validation",
