...
- Ensure correct credentials`,
        keywords: ["connection", "connect", "database", "mysql", "postgresql", "sqlite"],
...
- Contact support`,
  ...
  private savePreferences(): void {
    localStorage.setItem("db-extension-help-preferences", JSON.stringify(this.preferences))
  }
  ...
  private saveAnalytics(): void {
    localStorage.setItem("db-extension-help-analytics", JSON.stringify(this.analytics))
  }
  ...
  markTopicAsViewed(topicId: string): void {
    ...
    this.recentlyViewed = [topic, ...this.recentlyViewed.filter((t) => t.id !== topicId)].slice(
      0,
      5
    )
    ...
    this.analytics.topicViews[topicId] = (this.analytics.topicViews[topicId] || 0) + 1
    this.saveAnalytics()
  }
