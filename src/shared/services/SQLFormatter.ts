...
export class SQLFormatter {
  ...
  private formatTokens(tokens: Token[], options: Required<FormatOptions>): string {
    let result = ""
    let indentLevel = 0
      ...
      switch (token.type) {
        case "keyword":
          ...
          if (this.isMainClauseKeyword(token.value)) {
            if (options.lineBreakBeforeKeywords && prevToken && prevToken.type !== "whitespace") {
  ...
  private formatOperator(token: Token, prevToken: Token | null, nextToken: Token | null): string {
    let value = token.value

    // Add spaces around operators
    if (this.needsSpaceAround(value)) {
      if (prevToken && !this.isWhitespace(prevToken) && prevToken.value !== "(") {
        value = ` ${value}`
      }
      if (nextToken && !this.isWhitespace(nextToken) && nextToken.value !== ")") {
        value += " "
      }
    } else if (value === ",") {
  ...
  private readWhitespace(sql: string, startIndex: number): { value: string; newIndex: number } {
    let i = startIndex
  ...
  private readNumber(sql: string, startIndex: number): { value: string; newIndex: number } {
    let i = startIndex
  ...
  private readOperator(sql: string, startIndex: number): { value: string; newIndex: number } {
  ...
  private readIdentifier(sql: string, startIndex: number): { value: string; newIndex: number } {
    let i = startIndex
  ...
  private shouldIncreaseIndent(keyword: string): boolean {
    const indentKeywords = ["SELECT", "FROM", "WHERE", "GROUP", "HAVING", "ORDER"]
