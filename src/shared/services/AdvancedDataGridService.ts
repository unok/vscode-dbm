...
export class AdvancedDataGridService {
  ...
  async commitCellEdit(
    ...
    const validation = await this.validateCellValue(rowIndex, columnId, newValue)
    ...
    // Track the change
    this.changeTracker.recordCellChange(rowIndex, columnId, cellState.originalValue, newValue)
   ...
   */
  async executeBulkOperation(operation: BulkEditOperation): Promise<BulkOperationResult> {
  ...
  private async executeBulkDelete(operation: BulkEditOperation): Promise<number> {
    for (const rowIndex of operation.rowIndices) {
    ...
    try {
      ...
      for (let dataRowIndex = 0; dataRowIndex < copyData.data.length; dataRowIndex++) {
        ...
        for (let dataColIndex = 0; dataColIndex < dataRow.length; dataColIndex++) {
          ...
          // Validate the value
          const validation = await this.validateCellValue(targetRowIndex, targetColumn.id, newValue)
          if (!validation.isValid) {
   ...
   */
  async generateAIDefaults(options: CursorAIDefaultOptions): Promise<Record<string, CellValue>> {
