...
export class CellValidationEngine {
  ...
  async validateValue(
    ...
    try {
      ...
      if (value === null || value === undefined || value === '') {
        ...
        if (context) {
          const businessValidation = await this.validateBusinessLogic(value, column, context)
          errors.push(...businessValidation.errors)
          warnings.push(...businessValidation.warnings)
        }

        // Custom rules validation
        const customValidation = await this.validateCustomRules(value, column, context)
  ...
  private async validateDataType(value: CellValue, column: ColumnDefinition): Promise<{ errors: string[], warnings: string[] }> {
    const errors: string[] = []
    const warnings: string[] = []
    ...
    switch (true) {
      case columnType.includes('int'):
        if (!this.isInteger(value)) {
          ...
          if (columnType.includes('tinyint') && (numValue < -128 || numValue > 127)) {
            errors.push('Value must be between -128 and 127 for TINYINT')
          } else if (columnType.includes('smallint') && (numValue < -32768 || numValue > 32767)) {
            errors.push('Value must be between -32,768 and 32,767 for SMALLINT')
          } else if (columnType.includes('bigint') && (numValue < -9223372036854775808 || numValue > 9223372036854775807)) {
      ...
      case columnType.includes('varchar') || columnType.includes('char') || columnType.includes('text'):
  ...
  private validateLength(value: CellValue, column: ColumnDefinition): { errors: string[], warnings: string[] } {
    const errors: string[] = []
    const warnings: string[] = []
    ...
    if (typeof value === 'string') {
      ...
      if (maxLength && value.length > maxLength * 0.9) {
        warnings.push(`Value is close to maximum length limit (${value.length}/${maxLength})`)
      }
    }
  ...
  private validateFormat(value: CellValue, column: ColumnDefinition): { errors: string[], warnings: string[] } {
    const errors: string[] = []
    const warnings: string[] = []
  ...
  private async validateBusinessLogic(
    ...
    // Age validation
    if (column.name.toLowerCase().includes('age') && typeof value === 'number') {
      if (value < 0) {
    ...
    if (column.name.toLowerCase().includes('email') && context.allRows) {
      const duplicateEmails = context.allRows.filter((row, index) => 
        index !== context.rowIndex && row[column.id] === value
      )
    ...
    if (column.type.toLowerCase().includes('date')) {
      ...
      if (column.name.toLowerCase().includes('created') || column.name.toLowerCase().includes('start')) {
        if (dateValue > new Date(now.getTime() + 24 * 60 * 60 * 1000)) { // More than 1 day in future
    ...
    if ((column.name.toLowerCase().includes('salary') || 
         column.name.toLowerCase().includes('price') ||
         column.name.toLowerCase().includes('amount')) && typeof value === 'number') {
      if (value < 0) {
  ...
  private async validateCustomRules(
    ...
    for (const rule of rules) {
      try {
        switch (rule.type) {
          ...
          case 'custom':
            if (rule.validator && context) {
              const isValid = await Promise.resolve(rule.validator(value, context.row))
              if (!isValid) {
                errors.push(rule.message)
              }
            }
        ...
        warnings.push(`Custom validation error: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
  ...
  private async getSuggestions(value: CellValue, column: ColumnDefinition, errors: string[]): Promise<string[]> {
    ...
    if (errors.some(e => e.includes('email'))) {
      if (typeof value === 'string') {
        ...
        if (!value.includes('.')) {
          suggestions.push('Email must contain a domain with extension (e.g., .com)')
        }
      }
    }
    ...
    if (errors.some(e => e.includes('integer'))) {
      if (typeof value === 'string' && /^\d+\.\d+$/.test(value)) {
        suggestions.push(`Try ${Math.round(Number(value))} (rounded to nearest integer)`)
      }
    }
  ...
  private isBoolean(value: CellValue): boolean {
    return typeof value === 'boolean' ||
           (typeof value === 'string' && ['true', 'false', '1', '0'].includes(value.toLowerCase())) ||
  ...
  private validateJSON(value: CellValue): { errors: string[], warnings: string[] } {
    const errors: string[] = []
    const warnings: string[] = []
  ...
  private validateDecimal(value: CellValue, precision?: number, scale?: number): { errors: string[], warnings: string[] } {
    const errors: string[] = []
    const warnings: string[] = []
  ...
  private getCacheKey(value: CellValue, column: ColumnDefinition, context?: ValidationContext): string {
    const contextKey = context ? `${context.rowIndex}:${JSON.stringify(context.row)}` : 'no-context'
