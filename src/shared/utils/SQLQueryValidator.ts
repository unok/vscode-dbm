...
export class SQLQueryValidator {
  ...
  constructor(schema: DatabaseSchema) {
    ...
    this.sqlKeywords = new Set([
      ...
      'ELSE', 'END', 'IF', 'WHILE', 'FOR', 'DECLARE', 'BEGIN', 'COMMIT',
      'ROLLBACK', 'TRANSACTION', 'START', 'SAVEPOINT', 'GRANT', 'REVOKE'
    ])

    this.dangerousKeywords = new Set([
      'DROP', 'TRUNCATE', 'DELETE', 'ALTER', 'CREATE', 'GRANT', 'REVOKE',
      'EXEC', 'EXECUTE', 'SHUTDOWN', 'SCRIPT'
    ])
  ...
  private columnExists(columnName: string, tableAlias?: string | null): boolean {
    ...
    const table = this.schema.tables.find(t => 
      t.name.toLowerCase() === tableAlias.toLowerCase()
    )
