...
export class VirtualScrollManager {
  ...
  getItemsInRange(
    ...
    for (
      let index = start;
      index <= end && index < this.state.totalItems;
      index++
    ) {
      items.push({
        index,
  ...
  scrollToIndex(
    ...
    switch (alignment) {
      case "center":
        targetScrollTop =
          itemOffset - (this.state.containerHeight - itemHeight) / 2;
        break;
      case "end":
    ...
    return Math.max(
      ...
      Math.min(
        targetScrollTop,
        this.getTotalHeight() - this.state.containerHeight,
      ),
    );
  }
  ...
  getVisibleItems(): Array<{
    ...
    for (
      ...
      index <= range.end && index < this.state.totalItems;
      index++
    ) {
      const offset = this.getOffsetForIndex(index);
      const height = this.getItemHeight(index);
      const isVisible =
        index >= range.visibleStart && index <= range.visibleEnd;
