...
export class DataChangeTracker {
  ...
  recordCellChange(
    ...
    const actualOriginal = existingChange
  ...
  recordRowDeletion(rowIndex: number, data: Record<string, CellValue>): void {
    ...
    const primaryKeyColumn = this.originalData?.columns.find(
  ...
  getChangeRecord(): ChangeRecord {
    ...
    return {
      ...
      affectedRows,
      totalChanges:
        modifiedCells.length + addedRows.length + deletedRows.length,
    ...
    for (const [rowIndex, updates] of rowUpdates) {
      ...
      if (primaryKeyColumn) {
        ...
        if (originalRow && originalRow[primaryKeyColumn.id] !== undefined) {
          ...
          statements.push(
            `UPDATE ${tableName} SET ${setPairs} WHERE ${whereClause}`,
          );
        }
      }
    }
    ...
    for (const deletedRow of changeRecord.deletedRows) {
      ...
      if (primaryKeyColumn) {
        ...
        const primaryKeyValue =
          deletedRowWithPk.primaryKeyValue ||
          deletedRow.data[primaryKeyColumn.id];
    ...
    for (const addedRow of changeRecord.addedRows) {
      const columns =
        this.originalData?.columns
          .filter(
            (col) =>
              !col.isAutoIncrement && addedRow.data[col.id] !== undefined,
          )
          .map((col) => col.id) || [];
  ...
  hasChanges(): boolean {
    return (
      this.cellChanges.size > 0 ||
      this.rowAdditions.size > 0 ||
      this.rowDeletions.size > 0
    );
  }
  ...
  getChangesSummary(): {
    ...
    if (stats.totalChanges > 0) {
      ...
      if (stats.modifiedCells > 0)
        parts.push(`${stats.modifiedCells} cell(s) modified`);
      if (stats.addedRows > 0) parts.push(`${stats.addedRows} row(s) added`);
      if (stats.deletedRows > 0)
        parts.push(`${stats.deletedRows} row(s) deleted`);
    ...
    if (stats.modifiedCells > 0) {
      details.push(
        `Modified ${stats.modifiedCells} cell(s) across ${stats.affectedRows} row(s)`,
      );
    }
  ...
  private getLastModifiedTime(): Date | null {
    const allTimestamps = [
      ...Array.from(this.cellChanges.values()).map(
        (change) => change.timestamp,
      ),
      ...Array.from(this.rowAdditions.values()).map(
        (addition) => addition.timestamp,
      ),
      ...Array.from(this.rowDeletions.values()).map(
