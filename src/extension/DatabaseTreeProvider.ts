  ...
  implements vscode.TreeDataProvider<DatabaseTreeItem>
{
  ...
  constructor() {
    ...
    this.databaseService.addMessageListener("treeProvider", (message) => {
      if (
        message.type === "connectionStatus" ||
        message.type === "activeConnections"
      ) {
        this.refresh();
      }
    });
  ...
  private async getSchemaChildren(
    ...
    try {
      ...
      if (!schema) {
        // アクティブ接続が存在するかチェック
        const activeConnection = this.databaseService.getConnection(
          connection.id,
        );
      ...
      if (tableData.length > 0) {
        const tableItems = tableData.map(
          (table) => new TableTreeItem(table.name, connection.id),
        );
      ...
      if (viewData.length > 0) {
        const viewItems = viewData.map(
          (view) => new ViewTreeItem(view.name, connection.id),
        );
