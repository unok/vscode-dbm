  ...
  try {
    ...
    context.subscriptions.push(
      vscode.window.registerWebviewViewProvider(
        DatabaseWebViewProvider.viewType,
        webViewProvider,
      ),
    );
      ...
      async (treeItem: any) => {
        ...
        if (webViewProvider) {
          ...
          if (tableName) {
            ...
            console.error("Failed to extract table name from:", {
              tableName: treeItem?.tableName,
    ...
    const runQueryInPanelCommand = vscode.commands.registerCommand(
      ...
      async () => {
        ...
        if (activeConnections.length > 0) {
          ...
          // 選択された接続で自動接続
          const selectedConfig = savedConnections.find(
            (c) => c.id === selected.connectionId,
          );
          if (selectedConfig) {
            const connectionData = {
              ...
              ssl:
                typeof selectedConfig.ssl === "boolean"
                  ? selectedConfig.ssl
                  : false,
            };
        ...
        await databaseService.executeQueryWithResults(
          query.trim(),
          connectionId,
        );
      },
    ...
    const newSqlFileCommand = vscode.commands.registerCommand(
      "vscode-dbm.newSQLFile",
      async () => {
        const savedConnections = databaseService.getSavedConnections();
    ...
    console.error(
      "Failed to activate Database Manager (DBM) extension:",
      error,
    );
    vscode.window.showErrorMessage(
